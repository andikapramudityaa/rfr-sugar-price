...
def get_dataset():
    file_path = "../dataset/garlic_prices.xlsx"
...
def cleanup_dataset(df):
    ...
    plt.title("Garlic Price History")
    plt.xlabel("Date")
    plt.ylabel("Price (Rp/kg)")
    plt.grid(True, which="both", axis="both", linestyle="--", linewidth=0.5)
    plt.tight_layout()
    plt.savefig("../output/images/garlic_price_history.png", dpi=300)
    plt.show()
...
def feature_engineering(data):
    ...
    data.to_csv("../output/csv/rfr_feature_garlic.csv", index=False)
...
def do_forecast(model, data, feature_cols, n_year):
    ...
    plt.title("Garlic Price - History & Forecast")
    plt.xlabel("Date")
    plt.ylabel("Price (Rp/kg)")
    plt.legend()
    plt.tight_layout()
    plt.savefig("../output/images/forecast_future_garlic_prices.png", dpi=300)
    plt.show()
...
def compare_prediction(data_full, target_year):
    ...
    plt.title(f"Garlic Price: Prediction vs Actual ({target_year})")
    plt.xlabel("Date")
    plt.ylabel("Price (Rp/kg)")
    plt.legend()
    ...
    plt.tight_layout()
    plt.savefig(
        f"../output/images/compare_garlic_actual_vs_predicted_{target_year}.png",
        dpi=300,
    )
    plt.show()
